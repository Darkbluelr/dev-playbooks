...
usage() {
  cat <<'EOF' >&2
usage: ssot-index-sync.sh [options]

Apply a minimal ssot delta file to update requirements.index.yaml deterministically.

Options:
  --project-root <dir>     Project root directory (default: pwd)
  --truth-root <dir>       Truth root directory (fallback; default: specs)
  --change-root <dir>      Change root directory (used by --refresh-ledger; default: changes)
  --delta <path>           Path to inputs/ssot.delta.yaml (required)
  --set-ref <truth://...>  Override target_set_ref (default: from delta; fallback: truth://ssot/requirements.index.yaml)
  --apply                  Apply changes (default: dry-run summary only)
  --refresh-ledger         After sync, refresh requirements.ledger.yaml (derived cache)
  -h, --help               Show this help message

Delta contract (supported subset):
  ...
  ops:
    - op: add|update|remove
      ...
      anchor: "<path>#<anchor>"        # optional
      statement: "<single-line text>"  # required for add; optional for update

Notes:
- No external deps (no yq). Fields must be single-line scalars.
- Failure is non-destructive: the script refuses to write a broken index.
EOF
}
...
if [[ -z "$project_root" ]]; then
  die_usage "--project-root is required"
fi
...
if [[ "$meta_schema" != "1.0.0" ]]; then
  die_usage "ssot.delta.yaml schema_version must be 1.0.0 (actual: ${meta_schema:-<empty>})"
fi
...
if [[ "$set_ref" != truth://*requirements.index.yaml && "$set_ref" != truth://*requirements.index.yml ]]; then
  die_usage "--set-ref/target_set_ref must end with requirements.index.yaml|yml (actual: ${set_ref})"
fi
...
if [[ ! -f "$index_file" ]]; then
  die_usage "requirements.index not found: ${set_ref} (resolved: ${index_file})"
fi
if ! index_schema_ok "$index_file"; then
  die_usage "requirements.index schema_version must be 1.0.0: ${index_file}"
fi
...
validate_no_tabs() {
  ...
  local value="$2"
  if [[ "$value" == *$'\t'* ]]; then
    die_usage "${label} must not contain tab characters"
  fi
}
...
while IFS=$'\x1f' read -r rec op id severity anchor statement title has_sev has_anchor has_stmt has_title; do
  ...
  case "$op" in
    add|update|remove) ;;
    "") die_usage "op is required for each delta item" ;;
    *) die_usage "unsupported op: ${op}" ;;
  esac

  case "$op" in
    remove)
      if [[ -z "$id" ]]; then
        die_usage "remove op requires id"
      fi
      if [[ "$existing_ids_pipe" != *"|${id}|"* ]]; then
        die_usage "remove id not found in requirements.index: ${id}"
      fi
      append_op "remove" "$id" "" "" "" "0" "0" "0"
      ;;
    update)
      if [[ -z "$id" ]]; then
        die_usage "update op requires id"
      fi
      if [[ "$existing_ids_pipe" != *"|${id}|"* ]]; then
        die_usage "update id not found in requirements.index: ${id}"
      fi
      if [[ "$has_sev" != "1" && "$has_anchor" != "1" && "$has_stmt" != "1" ]]; then
        die_usage "update op must specify at least one of severity/anchor/statement: id=${id}"
      fi
      if [[ "$has_sev" == "1" ]]; then
        case "$severity" in
          must|should) ;;
          "") die_usage "update severity present but empty: id=${id}" ;;
          *) die_usage "update severity invalid (must|should): id=${id} severity=${severity}" ;;
        esac
      fi
      if [[ "$has_stmt" == "1" && -z "$statement" ]]; then
        die_usage "update statement present but empty: id=${id}"
      fi
    ...
    add)
      ...
      if [[ "$existing_ids_pipe" == *"|${id}|"* || "$new_ids_pipe" == *"|${id}|"* ]]; then
        die_usage "add id already exists: ${id}"
      fi
      ...
      if [[ "$has_sev" == "1" ]]; then
        case "$severity" in
          must|should) ;;
          "") die_usage "add severity present but empty: id=${id}" ;;
          *) die_usage "add severity invalid (must|should): id=${id} severity=${severity}" ;;
        esac
      else
        severity="must"
        has_sev="1"
      ...
      if [[ "$has_stmt" != "1" || -z "$statement" ]]; then
        die_usage "add op requires statement: id=${id}"
      fi
...
if [[ "$op_count" -eq 0 ]]; then
  die_usage "no ops found in delta: ${delta_path}"
fi

echo "info: ssot_index_sync" >&2
echo "  project_root: ${project_root}" >&2
echo "  truth_dir:    ${truth_dir}" >&2
echo "  set_ref:      ${set_ref}" >&2
echo "  index_file:   ${index_file}" >&2
echo "  ops_count:    ${op_count}" >&2
if [[ -n "${truth_mapping_json:-}" && "${truth_mapping_json}" != "'{}'" && "${truth_mapping_json}" != "{}" ]]; then
  echo "  truth_mapping: ${truth_mapping_json}" >&2
fi

if [[ "$apply" != true ]]; then
  echo "dry-run: no changes written (pass --apply to write)" >&2
  exit 0
fi
...
if [[ ! -s "$tmp_out" ]]; then
  ...
  die_usage "generated empty index output; refusing to overwrite: ${index_file}"
fi

mv "$tmp_out" "$index_file"
echo "ok: updated requirements.index: ${index_file}" >&2
...
if [[ "$refresh_ledger" == true ]]; then
  ...
  if [[ -f "$ledger_script" ]]; then
    bash "$ledger_script" \
      ...
      --set-ref "$set_ref" \
      >/dev/null
    echo "ok: refreshed derived ledger" >&2
  else
    echo "warn: ledger derive script not found; skipped refresh" >&2
  fi
fi
