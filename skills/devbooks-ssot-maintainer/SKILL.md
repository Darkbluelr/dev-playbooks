...
- `<devbooks-root>/ssot/requirements.index.yaml` (truth: stable id → anchor → statement)
- `<devbooks-root>/ssot/requirements.ledger.yaml` (derived cache: progress view; disposable/rebuildable)
Boundaries:
...
- **Not responsible for SSOT initialization** (use `brownfield-bootstrap` for initialization).
...
## Responsibility Boundary with brownfield-bootstrap

| Skill | Responsibility | Analogy | Trigger |
|-------|----------------|---------|---------|
| **brownfield-bootstrap** | **Create** SSOT skeleton | `git init` | `ssot/` doesn't exist or is empty |
| **ssot-maintainer** | **Maintain** SSOT (add/update/remove) | `git commit` | SSOT exists, needs changes |

**Key Distinction**:
- If `<devbooks-root>/ssot/` directory doesn't exist or is empty, use `brownfield-bootstrap` to initialize first
- This Skill only handles **add/update/remove of existing SSOT**, not creation from scratch
...
1. `.devbooks/config.yaml` (if present) → parse mappings (especially `paths.ssot`)
...
- If you do not have an upstream SSOT: ensure `<devbooks-root>/ssot/SSOT.md` and `<devbooks-root>/ssot/requirements.index.yaml` exist
  - If they don't exist, prompt user to run `brownfield-bootstrap` first
