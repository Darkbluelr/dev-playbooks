...
0) Check system dependencies:
   ...
   - If required deps are missing (jq, rg), instruct the user to run: ./setup/install-dependencies.sh
...
1) Identify the context protocol type (at least two branches):
   - If DevBooks is detected (dev-playbooks/project.md exists): install using templates under `setup/dev-playbooks/`.
   - Otherwise: install using `setup/template/DevBooks-Integration-Template.md`.
